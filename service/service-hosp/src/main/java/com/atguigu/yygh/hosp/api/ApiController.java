...
//		}
	...
	@DeleteMapping("confirmGetNumber")
	public Result batchRemove(HttpServletRequest request){
		Map<String, Object> paramMap = HttpRequestHelper.switchMap(request.getParameterMap());
		//必须参数校验 略
		String hoscode = (String)paramMap.get("hoscode");
		//必填
		String hosRecordId = (String)paramMap.get("hosRecordId");
		if(StringUtils.isEmpty(hoscode) || StringUtils.isEmpty(hosRecordId)) {
			throw new YyghException(ResultCodeEnum.PARAM_ERROR);
		}
		//签名校验
		if(!HttpRequestHelper.isSignEquals(paramMap, hospitalSetService.getSignKey(hoscode))) {
		...
		boolean isConfirm = orderInfoFeignClient.confirmGetNumber(hoscode, hosRecordId);
		if(!isConfirm) {
			Result.fail();
		}
		return Result.ok();
